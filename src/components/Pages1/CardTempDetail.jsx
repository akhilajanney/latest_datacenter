...
export default class CardTempDetail extends Component {
  ...
  tempDetails = () => {
    ...
    if ($("#filter").val() === "Rack") {
        ...
        .then((response) => {
          ...
          if (data.length !== 0 && response.status === 200) {
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              $("#" + data[i].macid).on("click", () => this.getChartData("rack", data[i].macid));
            }
      ...
      axios({ method: "GET", url: assettemp })
        .then((response) => {
          const data = response.data;
          console.log(new Date(), "Asset Response====>", response);
          $("#table_det tbody").empty();
          $("#table_det thead").empty();
          if (data.length !== 0 && response.status === 200) {
            $("#temp_details").show();
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              $("#" + data[i].tagid).on("click", () => this.getChartData("asset", data[i].tagid));
            }
  ...
  rackChartData = (macid) => {
    ...
    axios({ method: 'GET', url: '/api/rack/average?id=' + macid })
      .then((response) => {
        console.log(new Date(), "rackChartData====>", response)
  ...
  assetChartData = (tagid) => {
    ...
    axios({ method: "GET", url: "/api/track?id=" + tagid })
      .then((response) => {
        console.log(new Date(), "assetChartData response====>", response);
        let data = response.data
        if (data.length !== 0 && response.status === 200) {
          for (let i = 0; i < data.length; i++) {
